...
export default function ModelList(props: {
  ...
  onEditPath: (orderId: string, editor: (steps: PathStep[]) => PathStep[]) => void;
    ...
    <div className="rounded-lg border overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {sorted.map((o) => {
            ...
            return (
              <tr key={o.id} className="border-t">
                ...
                <td className="p-3">{i < 0 ? "Not started" : i >= o.steps.length ? "Completed" : step.kind === "machine" ? step.machineType : `Job Work`}</td>
                <td className="p-3">
                  {i < 0 || i >= o.steps.length ? (
                    <Badge variant="secondary">—</Badge>
                  ) : (
                    <Badge variant={step.status === "running" ? "default" : step.status === "hold" ? "destructive" : "secondary"}>
                      {step.status}
                    </Badge>
                  )}
                </td>
                <td className="p-3 max-w-[320px]">
                  <div className="flex flex-wrap gap-1">
                    {o.steps.map((s, idx) => (
                      <span key={s.id} className={`rounded-full px-2 py-0.5 text-xs border ${idx < i ? "bg-green-100 border-green-200 text-green-800 dark:bg-green-900/30 dark:text-green-300" : idx === i ? "bg-amber-100 border-amber-200 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300" : "text-muted-foreground"}`}>
                ...
                <td className="p-3">
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="secondary" onClick={() => setEditing(o)}>Edit Path</Button>
                    <Button size="sm" onClick={() => props.onNext(o.id)}>
                      <SkipForward className="h-4 w-4 mr-1"/> Next
                    </Button>
                    <Button size="icon" variant="ghost" onClick={() => setSplitFor(o)} title="Split into batches">
                      <Scissors className="h-4 w-4"/>
                    </Button>
                    <Button size="icon" variant="ghost" onClick={() => props.onDelete(o.id)} title="Delete">
          ...
          {sorted.length === 0 && (
            <tr>
              <td colSpan={6} className="p-6 text-center text-muted-foreground">No models yet. Create one to get started.</td>
            </tr>
          )}
        </tbody>
      </table>
              ...
              <div className="rounded-md border divide-y">
                {editing.steps.map((st, idx) => (
                  <div key={st.id} className="flex items-center gap-3 p-2">
                    <div className="min-w-8 text-xs text-muted-foreground text-center">{idx + 1}</div>
                    <div className="flex-1">
                      <div className="font-medium">{st.kind === "machine" ? st.machineType : "Job Work"}</div>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" onClick={() => props.onEditPath(editing.id, (steps) => {
                        const j = idx - 1; if (j < 0) return steps; const arr = steps.slice(); const tmp = arr[idx]; arr[idx] = arr[j]; arr[j] = tmp; return arr; })}><ArrowUp className="h-4 w-4"/></Button>
                      <Button size="icon" variant="ghost" onClick={() => props.onEditPath(editing.id, (steps) => {
                        const j = idx + 1; if (j >= steps.length) return steps; const arr = steps.slice(); const tmp = arr[idx]; arr[idx] = arr[j]; arr[j] = tmp; return arr; })}><ArrowDown className="h-4 w-4"/></Button>
                    </div>
                  </div>
                ))}
              </div>
