    ...
    <div className="min-h-screen">
      <div className="px-0 max-w-full">
        <div className="space-y-3">
          ...
          {/* Full-bleed desktop LIST view */}
          {viewMode === "list" && (
            <div className="hidden lg:block w-full">
              <div className="w-full">
                {/* Header row */}
                <div className="flex items-center justify-between px-4 py-2 bg-gray-50 dark:bg-gray-800 text-sm text-gray-700">
                  <div className="flex items-center gap-4">
                    {showDetails && <div style={{ width: 80 }}>Date</div>}
                    <div style={{ width: 240 }} className="font-medium">
                      Model
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div style={{ width: 240 }} className="font-medium text-right">
                      Current
                    </div>
                  </div>
                </div>
                ...
                {sorted.map((o) => {
                  ...
                  return (
                    <div key={o.id} className={`${bg}`} data-order-id={o.id} data-parent-id={o.parentId ?? ""}>
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-4">
                          {showDetails && (
                            <div style={{ width: 80 }} className="text-sm text-gray-700 dark:text-gray-300">
                              {formatDate(o.createdAt)}
                            </div>
                          )}

                          <div style={{ width: 240 }} className="flex items-center gap-2">
                            <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                              {o.modelName}
                            </div>
                            {o.quantity > 0 && (
                              <div className="text-sm text-muted-foreground">({o.quantity})</div>
                            )}
                          </div>
                        </div>

                        <div style={{ width: 240 }} className="text-sm text-right">
                          {/* Show all current machine names when expanded, otherwise show primary */}
                          {i < 0
                            ? "Not started"
                            : i >= o.steps.length
                              ? "Completed"
                              : isExpanded
                                ? (() => {
                                    const parallelGroup = (o.parallelGroups || []).find((g) => g.stepIndex === i);
                                    const selectedIndices = parallelGroup?.machineIndices || [];
                                    const primary = step.kind === "machine" ? step.machineType : "Job Work";
                                    const names = [primary, ...selectedIndices.map((idx) => machineTypes[idx]?.name).filter(Boolean)].filter(Boolean);
                                    return (
                                      <div className="flex flex-col items-end gap-0.5">
                                        {names.map((n) => (
                                          <div key={n} className="font-medium text-gray-900 dark:text-gray-100">{n}</div>
                                        ))}
                                      </div>
                                    );
                                  })()
                                : (() => {
                                    const primary = step.kind === "machine" ? step.machineType : "Job Work";
                                    return <div className="font-medium">{primary}</div>;
                                  })()}

                          {/* Status pill shown only when expanded, above job works */}
                          {isExpanded && (
                            <div className="mt-1">
                              <div className="flex justify-end">
                                <button onClick={() => toggleCardStatus(o)}>
                                  <Badge
                                    variant={
                                      (step?.status === "pending" ? "hold" : step?.status) === "running"
                                        ? "success"
                                        : (step?.status === "pending" ? "hold" : step?.status) === "hold"
                                          ? "destructive"
                                          : "secondary"
                                    }
                                    className="shrink-0 cursor-pointer"
                                  >
                                    {cap(step?.status === "pending" ? "hold" : step?.status)}
                                  </Badge>
                                </button>
                              </div>
                        ...
                        <div className="px-4 pb-3">
                          <div className="bg-muted/20 p-3 rounded-sm">
                            <div className="flex flex-col gap-2 text-sm text-muted-foreground">
                              <div>Date: {formatDate(o.createdAt)}</div>
                              {o.quantity > 0 && <div>Qty: {o.quantity}</div>}
                              <div className="flex flex-wrap items-center gap-1">{getPathLetterPills(o)}</div>
                              <div className="mt-2">
                                <Button size="sm" onClick={() => navigate(`/models/${o.id}/edit`)}>
                                  Open
                                </Button>
                              </div>
                            </div>
            ...
            {sorted.map((o) => {
              ...
              return (
                <div
                  ...
                  data-order-id={o.id}
                  data-parent-id={o.parentId ?? ""}
                  className={`${bg} ${viewMode === "cards" ? "rounded-lg p-4 space-y-3 shadow-sm border border-gray-200 dark:border-gray-800" : "py-2 px-0 space-y-1"} w-full ${bg ? "" : "bg-white dark:bg-gray-900"}`}
                >
